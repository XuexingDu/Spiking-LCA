...
# IF model
class IF(bp.dyn.NeuGroup):
  def __init__(self, size, lamb=0.5,**kwargs):
...
class SLCA_IF(bp.Network):
    def __init__(self, num, w, lamb = 0.5, scale=1.0, method='exp_auto'):
        super(SLCA_IF, self).__init__()
        
        # network size
        num_neuron = int(num * scale)
        self.N   = IF(num_neuron, lamb)

        # synapses
        w_matrix = w   # excitatory synaptic weight (voltage)
        # w_matrix  = 0.
...
class SLCA_WB(bp.DynamicalSystemNS):
    ...
    # def reversed_function(self,x):
    #     return 0.02254245* x**3 + 0.01746608 *x**2 + 0.26527295*x -0.02109287
